...
class KFoldWapper(object):
    ...
    def fit(self, x, y, candidate_idx, generated_k):
        ...
        for k in range(self.n_fold):
            ...
            if len(candidate_idx)!=0:
                generated_x = x[self.get_sample_idx(candidate_idx, generated_k)]
            else:
                generated_x = None

            est.fit(x[train_id], y[train_id], generated_x)
    ...
    def get_sample_idx(self,candidate_idx,generated_k):
        np.random.seed(self.random_state)
        idx = np.random.choice(candidate_idx, generated_k*len(candidate_idx), replace=True)
